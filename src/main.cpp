...
void operate(int, void*)
{
	...
	namedWindow("Gablur", WINDOW_NORMAL);
	...
	namedWindow("src1s", WINDOW_NORMAL);
	imshow("src1s", UsmSrc);
	...
	//connectedDomains.clear();
	findConnectedDomain(UsmSrc, connectedDomains, 300, 3);
	namedWindow("src4", WINDOW_NORMAL);
	imshow("src4", UsmSrc);
	...
	namedWindow("src5", WINDOW_NORMAL);
	imshow("src5", UsmSrc);
...
int main()
{
	string srcPath;
	//cout << "请输入图片路径：" << endl;
	//srcPath = "Source\\5.5\\W3.2.png";
	srcPath = "Source/5.12/block/block1.2.png";
	//Normal13.4.png
